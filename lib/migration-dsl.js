...
function MigrationDSL(driver) {
  this.driver           = driver;
  this.Dialect          = require("sql-ddl-sync/lib/Dialects/" + driver.dialect);
...
MigrationDSL.prototype = {
  ...
  addColumn: function (collectionName, options, cb) {
    ...
    this.Dialect.addCollectionColumn(this.driver, collectionName, column.value, options[columnName]['afterColumn'], cb);
  },
  ...
  execQuery: function (query, args, cb) {
    if (arguments.length === 2){
        // some queries don't have any args
        cb = args;
        args = {};
    }
